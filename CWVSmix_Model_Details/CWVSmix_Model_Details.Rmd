...
## Alternate Likelihood: Conditional Logistic Regression
For $1{:}1$ matched case-control data, let $l=1,...,L$ index matched pairs, with $\textbf{x}_{li}$ and $\textbf{z}_{li}$ denoting the covariate vectors for subject $i \in \{0,1\}$ (control, case) within pair $l$. The conditional likelihood, obtained by conditioning on the event that exactly one member of each pair is a case, is given as:

\begin{align*}
P\left(Y_{l1} = 1 | Y_{l0} + Y_{l1} = 1\right) &= \frac{\exp\left\{\eta_{l1}\right\}}{\exp\left\{\eta_{l1}\right\} + \exp\left\{\eta_{l0}\right\}},\ l=1,...,L; \\
\eta_{li} &= \textbf{x}_{li}^{\text{T}} \boldsymbol{\beta} + \sum_{t=1}^{m_l} \left[\sum_{j=1}^q \lambda_j\left(t\right) \text{z}_{lij}\left(t\right) + \sum_{j=1}^{q-1} \sum_{k=j+1}^q \widetilde{\lambda}_{jk}\left(t\right) \text{z}_{lij}\left(t\right)\text{z}_{lik}\left(t\right)\right]\alpha\left(t\right),\ i \in \{0,1\}.
\end{align*}

\begin{itemize}

\item $\boldsymbol{\beta}$ excludes any intercept term (not identifiable under conditional likelihood, since it cancels within each matched pair);

\item No additional parameters are introduced for this likelihood.
...
* likelihood_indicator $= 3$: Conditional logistic regression (requires `pair_id`, a vector of integers identifying $1{:}1$ matched pairs).
